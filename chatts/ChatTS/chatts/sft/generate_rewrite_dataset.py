...
"""
    Main function for time series rewrite.
    Usage:
        1. Set MODEL_PATH to the path of the model and set num_gpus, gpu_per_model accordingly.
        2. Run `python3 -m chatts.sft.generate_rewrite_dataset`.
...
import yaml
from typing import *
from chatts.utils.llm_utils import parse_llm_json
from chatts.sft.utils.rewrite_prompt import RewritePrompt
import copy
...
def worker_vllm(input_queue, validation_queue, input_response, validation_response, gpu_id, batch_size, finished_flag, model_path=MODEL_PATH):
    ...
    try:
        ...
        while not finished_flag.value:
            ...
            if batch_prompts:
                # Don't check finished_flag during processing - let the batch complete
                answers = llm.generate(batch_prompts, sampling_params, use_tqdm=False)
                answers = [i.outputs[0].text for i in answers]

                for i in range(len(answers)):
                    # Don't check finished_flag during result processing - complete the batch
...
def llm_batch_generate(seed_prompts: List[RewritePrompt], use_chat_template=True, num_gpus=num_gpus, batch_size=batch_size, model_path=MODEL_PATH, engine=ENGINE):
    manager = multiprocessing.Manager()
    input_queue = manager.Queue()
    input_response = manager.Queue()
    validation_queue = manager.Queue()
    output_list = manager.list()
    finished_flag = manager.Value('b', False)  # Boolean flag for graceful shutdown
            ...
            while success_cnt < TOTAL_CNT:
                ...
                while not input_response.empty():
                    ...
                    try:
                        ...
                        cur_seed_prompt: RewritePrompt = copy.deepcopy(cur_items[-1])
                ...
                for line in output_list[previous_output_len:current_output_len]:
                    ...
                    if 'valid' in line[0].lower() and 'invalid' not in line[0].lower():
                        ...
                        # Push to input queue
                        for _ in range(DFS_K):
                            item: RewritePrompt = copy.deepcopy(line[-1])
                            item.rewrite()
                ...
                pbar_input.set_description(f'[REWRITE] fail={parse_failed}')
                pbar_validation.set_description(f'[VALID] fail={validation_failed}')

        # Signal workers to finish gracefully
        finished_flag.value = True
        
        # Wait for processes to finish gracefully
        for p in processes:
            p.join(timeout=20)  # Wait up to 10 seconds for graceful shutdown
            if p.is_alive():
                logger.warning(f"Process {p.pid} did not finish gracefully, terminating...")
                p.terminate()
                p.join(timeout=5)
                if p.is_alive():
...
def evol_instruct():
    ...
    for input_file, label_file in INPUT_FILES:
        ...
        for data, label in zip(qa_dataset, labels):
            input_list.append(RewritePrompt(
                ts_idx=label['ts_idx'],
