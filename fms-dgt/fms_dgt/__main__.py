...
def add_base_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument(
        ...
        type=str,
        nargs="*",
        help="Additional namespaces to include in addition to the default.",
    )
    group.add_argument(
        "--config-path",
        type=str,
...
def main(
    ...
    # TODO: add other elements of base_args as needed
):
    base_args, builder_kwargs, task_kwargs = parse_cmd_line()

    if namespaces is None:
        # default should be empty for portability to destinations where no namespaces exist
        namespaces = []
