#!/bin/bash
#SBATCH --job-name=sdforger-synth-amp-seed
...
# Stage B replication seed (pass SEED via --export). New per-seed output dir.
set -eo pipefail
...
: "${SEED:?set SEED via sbatch --export}"
...
python scripts/run_synthetic_amplitude_control.py \
  ...
  --output-dir "output/time_series/pamap2_subject101_synthetic_amplitude_control_seed${SEED}_20260621" \
  --scale 3.0 --per-amp 40 --max-per-amp 80 \
  --generation-batch-size 64 --temperature 1.3 --seed "${SEED}"
