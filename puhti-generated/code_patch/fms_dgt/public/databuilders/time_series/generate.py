...
class TimeSeriesDataBuilder(TransformationDataBuilder):
    ...
    def __call__(
        ...
        target_llm_args = dict(self.config.target)
        target_llm_args.update(model_params.get("target_overrides", {}))
        trainer_model_id_or_path = model_params.get("model_id_or_path", self.trainer.model_id_or_path)
        trainer_overrides = model_params.get("training_overrides", {})
        ...
        train_params = sdforger_params | self.trainer.training_args | trainer_overrides
        ...
        conditioning_column = sdforger_params.get("conditioning_label_column")
        conditioning_value = sdforger_params.get("conditioning_label_value")
        if conditioning_column and conditioning_value:
            original_dataset[conditioning_column] = conditioning_value
            embedded_data = original_dataset.copy(deep=True)
        ...
        tuned_model_id_or_path = self.trainer(
            ...
            model_id_or_path=trainer_model_id_or_path,
            training_overrides=trainer_overrides,
        )
    ...
    def generate_data(
        ...
        return pd.concat(dfs).reset_index(drop=True)[numerical_columns].to_numpy()
