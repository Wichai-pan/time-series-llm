...
#                       DATA OBJECTS
# ===========================================================================
# paper-note-added: 配置数据类（框架管线，无直接论文映射）：target 持有用于微调/推理的 LLM 配置（Sec3.2.2 的 GPT-2 等）。
...
#                       MAIN CLASS
# ===========================================================================
# paper-note-added: 本类是 SDForger 六步流水线的总编排器（论文 Sec3），把分段/嵌入/文本编码/LLM微调与推理/文本解码/逆嵌入串起来。
# paper-note-added: 具体的 ICA/FPC 嵌入与逆嵌入、文本编解码由 utils.py 提供；本文件负责调用顺序、推理采样、生成内过滤(A.3)与停止准则(A.4)。
...
class TimeSeriesDataBuilder(TransformationDataBuilder):
    ...
    # paper-note-added: 单条时间序列的完整生成流程入口，对应论文六步流水线的整体串联（预处理->嵌入->微调->生成->解码）。
    def __call__(
        self,
        ...
        # paper-note-added: 第(1)步 周期感知分段（论文 A.1）：用 ACF 估计主周期 P，按步长 s 切出约 I≈15 个窗口实例，并做标准化；细节在 utils.preprocess_train_data。
        ...
        # paper-note-added: 第(2)步 时间序列->表格嵌入（论文 Sec3.1）：把每个窗口投影到 k 个基函数上，系数 e_ij=<X_i,b_j>，得到 IxK 嵌入表 E。
        # paper-note-added: 两种基的选择见 Sec3.1：FPC=泛函主成分（按方差排序、紧凑），FastICA=统计独立成分（无序、信息均摊、生成更鲁棒）。
        if train_params["embedding_type"] in ("fpc", "fpc-filled"):
            # paper-note-added: FPC 嵌入（协方差基，按方差排序）；embedding_dim/variance_explained 对应 A.2 选 k（最小 k 解释目标方差，K>25 易不稳）。
            (
                embedded_data,
                embedding_dims,
                data_embedded,
            ...
            # paper-note-added: FastICA 嵌入（独立成分基）；返回 mixing/mean 用于后续 Sec3.3 线性逆变换重建时间序列。
            (
                embedded_data,
                embedding_dims,
                data_embedded,
        ...
        # paper-note-added: 保留原始嵌入表副本，作为 A.3 过滤(原始 L2 范数分布)与 A.4 多样性参照的基准。
        # But first, record original training dataset and its properties
        original_dataset = embedded_data.copy(deep=True)

        # paper-note-added: 第(4)步前半 LLM 微调（论文 Sec3.2.2）：在文本化嵌入上微调 GPT-2（Adam lr8e-5、batch32、早停）；文本编码(Sec3.2.1)与训练在 trainer 块内。
        # Finetune model
        ...
        # paper-note-added: 第(4)步后半 推理采样（Sec3.2.2）+ 第(5)步文本解码 + A.3生成内过滤 + A.4停止准则，全部在 generate_data 内完成。
        ...
        # paper-note-added: 第(6)步 ICA/FPC 逆嵌入（论文 Sec3.3）：x_i = Σ_j e_ij·b_j，用生成系数与基函数线性组合重建时间序列。
        # Transform data back to the original feature space
        if train_params["embedding_type"] in ("fpc", "fpc-filled"):
            # paper-note-added: FPC 解码：生成系数与 FPC 基线性组合（Sec3.3 重建公式）。
            ...
            # paper-note-added: FastICA 解码：用 mixing/mean 把独立成分系数线性逆变换回原始特征空间（Sec3.3）。
    ...
    # paper-note-added: 第(4)推理采样 + (5)文本解码 + A.3生成内过滤(L2范数IQR) + A.4停止准则(多样性D) 的核心循环：用微调后的 LLM 自回归生成嵌入系数行，过滤后再交给逆嵌入。
    def generate_data(
        self,
        llm: LMProvider,
        sdforger_params: Dict,
        ...
        """
        # paper-note-added: A.4 停止准则相关阈值：min/max 输出数对应 I_max 上限与最少样本量；inference_batch 是每轮多项式采样的批大小。
        ...
        # paper-note-added: A.3 按通道(每个 k_c)拆分嵌入向量，逐通道做范数过滤；embedded_dims=各通道分量数，累加得切分点。
        ...
        # paper-note-added: A.3 计算原始嵌入向量的(每通道)L2范数分布，作为后续判定“发散实例”的参照区间基准（FPC下曲线L2范数≈系数欧氏范数）。
        old_l2_norms_splits = [
            list(np.linalg.norm(split.astype(float), axis=1)) for split in original_data_splits
        ]

        # paper-note-added: A.4 每通道的多样性分数 D 序列容器（唯一范数数/有效实例数），用于停止判定。
        norms_diversity = [[] for _ in embedded_dims]

        # paper-note-added: Sec3.2.1 文本编码（推理侧）：构造“填空(fill-in-the-middle)”提示，对 K 个特征做每实例随机置换 π 以消除位置/顺序偏置，blank 待 LLM 自回归填入系数。
        def init_tokens(
            tokenizer,
            permute: bool = True,
            ...
            # paper-note-added: Sec3.2.1 论文采用的 FIM 模板分支：每个批样本独立随机置换特征列顺序 π，再拼成 "Input: f is [blank], ... [sep] Target:"。
            elif text_template == "fim_template":
                col_starter = []
                if permute:
                    col_list = dataset_columns.copy()
                    for _ in range(0, generation_batch_size):
                        # paper-note-added: Sec3.2.1 关键——逐实例随机置换 π：打乱特征顺序以去除位置偏置（每个 batch 样本一个独立排列）。
                        random.shuffle(col_list)
                        col_starter.append(col_list.copy())

                else:
                    col_starter = [dataset_columns.copy()] * generation_batch_size
                # paper-note-added: Sec3.2.1 FIM 提示模板拼装：每个特征位置填 [blank]，[sep] 后接 Target:，等待模型把 [answer] 系数自回归填入。
                input_prompts = [
            ...
            # paper-note-added: Sec3.2.1 FIM 模板的条件式扩展：额外加 "Condition:" 段落注入分类特征，作为生成的条件上下文（数值特征仍走 [blank] 填空）。
                ...
                for _ in range(generation_batch_size):
                    ...
                    if permute:
                        col_list = dataset_columns.copy()
                        # paper-note-added: Sec3.2.1 同样的随机置换 π，应用于数值特征列以消除顺序偏置。
                        random.shuffle(col_list)
        ...
        try:
            ...
            # paper-note-added: A.4 生成主循环：持续采样直到满足停止准则或达到 max_outputs_to_generate(I_max) 上限。
            while (
                total_samples_generated < min_outputs_to_generate
                or total_samples_generated < max_outputs_to_generate
            ):
                # paper-note-added: Sec3.2.2 推理采样：把带 [blank] 的 FIM 提示喂给微调后的 LLM，由其多项式/温度采样自回归填入 [answer] 系数（采样参数在 LLM 块内配置）。
                llm_outputs = llm(
                    [
                        {
                            "input": prompt,
                ...
                # paper-note-added: 第(5)步 文本解码（Sec3.2.1 反向）：从生成文本中按特征名解析回数值系数，重建 IxK 嵌入表的一批行。
                ...
                # paper-note-added: A.3 过滤(1)——丢弃含缺失值/未生成完整的实例（"NaN"/空值行）。
                ...
                # paper-note-added: A.3 过滤(2)——跨全部已生成样本去重，丢弃重复实例（round(3)后 drop_duplicates）。
                ...
                # paper-note-added: A.3 过滤(3)——丢弃“发散实例”：用嵌入向量 L2 范数的 IQR 法则判离群（核心步骤）。
                if check_distribution:
                    ...
                    # paper-note-added: A.3 计算本批新样本每通道嵌入系数向量的 L2 范数（与原始范数分布对比）。
                    # Calculate L2 norms for each subset
                    new_generated_norms = [
                        np.linalg.norm(split, axis=1) for split in new_data_splits
                    ]

                    # paper-note-added: A.3 区间界 [q1-3*IQR, q3+3*IQR]：落在区间外即判为发散实例并丢弃（论文给定的 3*IQR 阈值）。
                    ...
                    # paper-note-added: A.3 逐通道做范数区间检验，所有通道都通过的样本才被接受（accepted_mask 逻辑与）。
                ...
                # paper-note-added: A.4 多样性分数 D = (四舍五入后唯一范数个数)/(有效实例总数)，逐通道计算（注：此处取整为3位小数，论文表述为4位）。
                ...
                # paper-note-added: 工程兜底：连续多轮无新增有效样本则提前终止（非论文核心准则，属于防卡死保护）。
                ...
                # paper-note-added: A.4 停止准则：当各通道多样性 D 的最大值 < λ_stop(norms_diversity_threshold)（即新样本范数已高度重复、信息饱和）时停止；或样本数超 I_max。
        ...
        # paper-note-added: 返回通过 A.3 过滤后的有效嵌入系数表(numpy)，作为第(6)步逆嵌入的输入。
        return pd.concat(dfs).reset_index(drop=True).to_numpy()

    # paper-note-added: 可视化辅助（非论文方法步骤）：把生成数据用 scaler 逆标准化后与原始序列对比作图，便于人工核验。
